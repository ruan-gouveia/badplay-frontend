...
export default function ConteudoDetalhesPage() {
  ...
  // O "Fragment" (<>) no return permite colocar os modais FORA do PageWrapper!
  return (
    <>
      <AnimatePresence>
        {/* MODAL DE UPGRADE CUSTOMIZADO (Livre do CSS do PageWrapper) */}
        {showUpgradeModal && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/80 backdrop-blur-md px-4">
            ...
            >
              ...
              <div className="flex gap-4 w-full">
                <button onClick={() => { setShowUpgradeModal(false); router.push("/catalogo"); }} className="flex-1 py-3 rounded-md font-semibold text-gray-300 bg-transparent border border-gray-700 hover:bg-gray-800 hover:text-white transition-colors">
                  Voltar
                </button>
                <button onClick={() => router.push("/planos")} className="flex-1 py-3 rounded-md font-semibold text-white bg-red-600 hover:bg-red-700 transition-colors shadow-[0_0_15px_rgba(220,38,38,0.4)]">
                  Fazer Upgrade
                </button>
              </div>
            </motion.div>
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/80 backdrop-blur-md px-4">
            ...
            >
              ...
              <textarea required value={editComentario} onChange={(e) => setEditComentario(e.target.value)}
                className="w-full bg-[#141414] text-white p-4 rounded-md border border-gray-700 focus:border-red-600 focus:outline-none resize-none h-32 mb-8 placeholder-gray-600"
              />
              
              <div className="flex gap-4 w-full">
                <button onClick={() => setShowEditModal(false)} className="flex-1 py-3 rounded-md font-semibold text-gray-300 bg-transparent border border-gray-700 hover:bg-gray-800 hover:text-white transition-colors">
                  Cancelar
                </button>
                <button onClick={handleSalvarEdicao} disabled={enviandoAvaliacao} className="flex-1 py-3 rounded-md font-semibold text-white bg-red-600 hover:bg-red-700 transition-colors disabled:opacity-50">
                  {enviandoAvaliacao ? "Salvando..." : "Salvar"}
                </button>
              </div>
            </motion.div>
          </div>
          ...
          <div className="mt-10 grid grid-cols-1 md:grid-cols-2 gap-10">
            ...
            <div>
              ...
              {minhaAvaliacaoExistente ? (
                <div className="bg-[#111111] rounded-2xl p-8 border border-gray-800 flex flex-col items-center justify-center text-center h-[280px]">
                  ...
                  <p className="text-sm text-gray-500">Para alterar, use os botões no seu comentário ao lado.</p>
                </div>
              ) : (
                <form onSubmit={handleAvaliar} className="bg-[#111111] rounded-2xl p-6 border border-gray-800">
                  ...
                  <textarea required value={meuComentario} onChange={(e) => setMeuComentario(e.target.value)} placeholder="O que você achou deste conteúdo?"
                    className="w-full bg-[#141414] text-white p-4 rounded-md border border-gray-700 focus:border-red-600 focus:outline-none resize-none h-32 mb-4 placeholder-gray-600"
                  />
                  <button type="submit" disabled={enviandoAvaliacao} className="w-full bg-red-600 hover:bg-red-700 text-white font-bold py-3 px-8 rounded-md transition-colors disabled:opacity-50">
            ...
            <div>
              ...
              <div className="flex flex-col gap-4 max-h-[400px] overflow-y-auto pr-2 scrollbar-hide">
                {avaliacoes.length === 0 ? (
                  <p className="text-gray-500 italic bg-[#111111] p-6 rounded-2xl border border-gray-800 text-center py-10">
                    ...
                    return (
                      <div key={av.id} className={`bg-[#111111] rounded-2xl p-6 border transition-colors ${isMeuComentario ? 'border-gray-600' : 'border-gray-800'}`}>
                        <div className="flex items-start gap-4">
                          <div className="w-10 h-10 rounded-full bg-red-600/20 border border-red-500/30 flex-shrink-0 flex items-center justify-center font-bold text-red-500 text-lg">
                          ...
                          <div className="flex-grow">
                            ...
                            {isMeuComentario && (
                              <div className="flex justify-end gap-5 mt-5 pt-4 border-t border-gray-800">
                                <button onClick={() => abrirModalEdicao(av)} className="text-gray-400 hover:text-white text-xs font-semibold uppercase tracking-wider transition-colors">
                                  Editar
                                </button>
                                <button onClick={() => handleDeletarAvaliacao(av.id)} className="text-red-500/80 hover:text-red-500 text-xs font-semibold uppercase tracking-wider transition-colors">
                                  Remover
                                </button>
                              </div>
                            )}
