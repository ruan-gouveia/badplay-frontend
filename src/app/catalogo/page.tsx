...
export default function CatalogoPage() {
  ...
  const handleRemoverHistorico = async (idHistorico: number, e: React.MouseEvent) => {
    e.stopPropagation(); // Impede que o clique no botão ative o clique do card (que abre o filme)
    try {
  ...
  return (
    <PageWrapper hasNavbar={true}>
      ...
      {/* 1. Componente Reutilizável de Banner */}
      {destaque && <BannerDestaque destaque={destaque} />}

      {/* Carrosséis */}
      <div className="relative z-20 pb-20 -mt-10 flex flex-col gap-10">
        
        {/* Histórico (Ainda customizado pois tem a barra de progresso e botões extras) */}
        {historico.length > 0 && (
        ...
        {/* 2. Componente Reutilizável de Cards para Filmes */}
        {filmes.length > 0 && (
          <div className="pl-6 md:pl-16">
        ...
        {/* 3. Componente Reutilizável de Cards para Séries */}
        {series.length > 0 && (
          <div className="pl-6 md:pl-16">
