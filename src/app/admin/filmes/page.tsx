...
export default function AdminFilmesPage() {
  ...
  const buscarNoTMDB = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.error("Erro ao comunicar com o TMDb. Verifique sua API Key.");
    } finally {
  ...
  const handleImportarFilme = async (tmdbFilme: TMDBResult) => {
    ...
    try {
      ...
      if (tmdbFilme.poster_path) {
        const imageUrl = `https://image.tmdb.org/t/p/w500${tmdbFilme.poster_path}`;
        imageFile = await urlToFile(imageUrl, `tmdb_${tmdbFilme.id}.jpg`, 'image/jpeg');
      }
      ...
      const dadosFilme = {
        ...
        planoMinimo: "BASICO", // Importa como básico por padrão!
      ...
      console.error(error);
      toast.error(error.message || "Erro ao importar filme.");
    } finally {
      setImportandoId(null);
    }
  };

  const handleDeletarFilme = async (id: number) => {
    if(!confirm("Deletar este filme excluirá ele de todas as listas e históricos. Confirmar?")) return;
    try {
  ...
  // NOVA FUNÇÃO: Atualiza o plano de um filme diretamente pela tabela
  const handleChangePlano = async (filme: Filme, novoPlano: string) => {
    try {
      // Monta os dados iguais aos originais, mas com o novo plano
      const dadosFilme = {
        titulo: filme.titulo,
        descricao: filme.descricao,
        anoLancamento: filme.anoLancamento,
        duracaoMinutos: filme.duracaoMinutos,
        trailerUrlYoutube: filme.trailerUrlYoutube,
        planoMinimo: novoPlano,
        generosIds: filme.generos.map(g => g.id) // Extrai apenas os IDs dos gêneros
      };

      const formData = new FormData();
      formData.append("dados", new Blob([JSON.stringify(dadosFilme)], { type: "application/json" }));

      // Envia PUT sem imagem (a imagem original é mantida no banco)
      const resp = await api.put(`/filmes/${filme.id}`, formData, {
        headers: { "Content-Type": "multipart/form-data" }
      });

      // Atualiza apenas a linha daquele filme na tabela
      setFilmes(filmes.map(f => f.id === filme.id ? resp.data : f));
      toast.success(`Plano do filme atualizado para ${novoPlano}!`);
    } catch (error) {
      toast.error("Erro ao alterar o plano do filme.");
    }
  };
    ...
    <>
      <CustomModal isOpen={showTmdbModal} title="Buscar Filme (TMDb)" maxWidth="max-w-4xl">
        <form onSubmit={buscarNoTMDB} className="flex gap-2 mb-6">
          <input 
            type="text" autoFocus placeholder="Digite o nome (Ex: Vingadores, Matrix)..." 
            value={query} onChange={(e) => setQuery(e.target.value)} 
            className="flex-grow bg-[#141414] text-white p-3 rounded-md border border-gray-700 focus:border-red-600 focus:outline-none" 
          />
          <LoadingButton type="submit" isLoading={buscandoTmdb} textLoading="Buscando..." className="!w-auto px-6">
        ...
        <div className="max-h-[500px] overflow-y-auto pr-2 scrollbar-hide">
          {resultadosTmdb.length === 0 && !buscandoTmdb ? (
            <p className="text-gray-500 text-center py-10">Pesquise por um filme para importar para o BadPlay.</p>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {resultadosTmdb.map(filme => (
                <div key={filme.id} className="bg-[#141414] border border-gray-800 rounded-lg p-3 flex gap-4 hover:border-gray-600 transition">
                  {filme.poster_path ? (
                    <img src={`https://image.tmdb.org/t/p/w200${filme.poster_path}`} alt="poster" className="w-20 h-30 object-cover rounded shadow-md" />
                  ) : (
                    <div className="w-20 h-30 bg-gray-900 rounded flex items-center justify-center text-gray-700"><ImageIcon /></div>
                  )}
                  ...
                  <div className="flex flex-col justify-between flex-grow overflow-hidden">
                    ...
                    <button 
                      onClick={() => handleImportarFilme(filme)}
                      disabled={importandoId === filme.id}
                      className="mt-2 text-xs font-bold text-red-500 hover:text-red-400 text-right uppercase disabled:opacity-50 flex justify-end items-center gap-2"
                    >
                      {importandoId === filme.id ? <><Loader2 className="w-3 h-3 animate-spin" /> Importando...</> : "Importar para o BadPlay"}
                    </button>
                  </div>
                </div>
              ))}
              ...
              <TableBody>
                {filmes.map((filme) => (
                  <TableRow key={filme.id} className="border-gray-800 hover:bg-gray-900/50">
                    ...
                    {/* CÉLULA DO PLANO COM SELECT INTERATIVO */}
                    <TableCell>
                      <div className="relative inline-block">
                        <select
                          value={filme.planoMinimo || "BASICO"}
                          onChange={(e) => handleChangePlano(filme, e.target.value)}
                          className={`appearance-none cursor-pointer pr-6 pl-2 py-1 rounded text-[10px] font-extrabold focus:outline-none focus:ring-1 focus:ring-red-600 transition-colors ${
                            filme.planoMinimo === 'PREMIUM' ? 'bg-yellow-500 text-black' : 
                            filme.planoMinimo === 'PADRAO' ? 'bg-blue-600 text-white' : 'bg-gray-700 text-white'
                          }`}
                        >
                        ...
                        {/* Setinha customizada do select para combinar com as cores */}
                        <div className="pointer-events-none absolute inset-y-0 right-1 flex items-center px-1">
                          <svg className={`fill-current h-3 w-3 ${filme.planoMinimo === 'PREMIUM' ? 'text-black' : 'text-white'}`} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20">
                    ...
                    <TableCell className="text-right">
                      <button onClick={() => handleDeletarFilme(filme.id)} className="text-gray-500 hover:text-red-500 p-2 transition">
                ...
                {filmes.length === 0 && (
                  <TableRow>
                    <TableCell colSpan={5} className="text-center py-10 text-gray-500">Nenhum filme cadastrado no banco local.</TableCell>
                  </TableRow>
                )}
              </TableBody>
            </Table>
