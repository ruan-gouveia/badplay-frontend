...
export default function AdminSeriesPage() {
  ...
  useEffect(() => {
    const carregar = async () => {
      try {
        const [respGen, respSer] = await Promise.all([api.get("/generos"), api.get("/series")]);
        setGenerosLocais(respGen.data);
        setSeries(respSer.data.sort((a: Serie, b: Serie) => b.id - a.id));
      } catch (error) { toast.error("Erro ao carregar banco."); }
  ...
  return (
    <>
      <ModalImportarSerie 
        ...
        onSuccess={(nova) => setSeries([nova, ...series])} 
      ...
      <ModalTemporadas 
        ...
        serie={serieEditando} 
        onSuccess={(atualizada) => setSeries(series.map(s => s.id === atualizada.id ? atualizada : s))}
      />
          ...
          <div className="bg-[#111111] rounded-xl border border-gray-800 overflow-hidden">
            <Table>
              <TableHeader className="bg-[#1a1a1a]">
                <TableRow className="border-gray-800"><TableHead>ID</TableHead><TableHead>Título</TableHead><TableHead>Temporadas</TableHead><TableHead>Plano Mínimo</TableHead><TableHead className="text-right">Ações</TableHead></TableRow>
              </TableHeader>
              <TableBody>
                {series.map((serie) => (
                  <TableRow key={serie.id} className="border-gray-800 hover:bg-gray-900/50">
                    ...
                    <TableCell>
                      <div className="relative inline-block">
                        <select
                          value={serie.planoMinimo || "BASICO"}
                          onChange={(e) => handleChangePlano(serie, e.target.value)}
                          className={`appearance-none cursor-pointer pr-6 pl-2 py-1 rounded text-[10px] font-extrabold focus:outline-none focus:ring-1 focus:ring-red-600 transition-colors ${
                            serie.planoMinimo === 'PREMIUM' ? 'bg-yellow-500 text-black' : 
                            serie.planoMinimo === 'PADRAO' ? 'bg-blue-600 text-white' : 'bg-gray-700 text-white'
                          }`}
                        >
                        ...
                        <div className="pointer-events-none absolute inset-y-0 right-1 flex items-center px-1">
                          <svg className={`fill-current h-3 w-3 ${serie.planoMinimo === 'PREMIUM' ? 'text-black' : 'text-white'}`} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20">
                    ...
                    <TableCell className="text-right">
                      <div className="flex items-center justify-end gap-1">
                        <button 
                          onClick={() => { setSerieEditando(serie); setShowTempModal(true); }} 
                        ...
                        <button 
                          onClick={() => { setIdParaDeletar(serie.id); setShowDeleteModal(true); }} 
                          className="text-gray-500 hover:text-red-500 p-2 transition"
                          title="Excluir Série"
                        >
