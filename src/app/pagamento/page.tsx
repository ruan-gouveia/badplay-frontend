...
function PagamentoForm() {
  ...
  const handlePagamento = async (e: React.FormEvent) => {
    ...
    try {
      // Pega o login temporário que salvamos na tela de Cadastro
      ...
      // 1. Faz Login na API para pegar o Token de segurança
      const respLogin = await api.post("/auth/login", { email, senha });
      const dadosLogin = respLogin.data;

      // Salva o Token definitivo no navegador
      ...
      // 2. Com o Token configurado, fazemos o POST da Assinatura!
      ...
      // 3. Limpa a memória temporária (segurança) e vai para o Catálogo!
  ...
  return (
    <div className="w-full max-w-md bg-black/80 p-10 rounded-xl shadow-2xl border border-gray-800 backdrop-blur-sm">
      ...
      <form onSubmit={handlePagamento} className="flex flex-col gap-4">
        ...
        <input type="text" placeholder="Nome no Cartão" required value={nomeTitular} onChange={(e) => setNomeTitular(e.target.value)} 
          className="w-full bg-[#333333] text-white p-4 rounded focus:outline-none focus:ring-2 focus:ring-red-600" />
        
        <input type="text" placeholder="Número do Cartão (16 dígitos)" required maxLength={16} minLength={16} value={numeroCartao} onChange={(e) => setNumeroCartao(e.target.value.replace(/\D/g, ''))} 
          className="w-full bg-[#333333] text-white p-4 rounded focus:outline-none focus:ring-2 focus:ring-red-600" />
        
        <input type="text" placeholder="CVV (3 dígitos)" required maxLength={3} minLength={3} value={cvv} onChange={(e) => setCvv(e.target.value.replace(/\D/g, ''))} 
          className="w-full bg-[#333333] text-white p-4 rounded focus:outline-none focus:ring-2 focus:ring-red-600" />
        ...
        <button type="submit" disabled={carregando} 
          ...
          {carregando ? "Processando..." : "Assinar e Assistir"}
        </button>
      </form>
    </div>
  );
